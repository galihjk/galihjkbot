...
**Diverifikasi lewat integration test ad-hoc** (SQLite file asli + `GameManager` sungguhan, `FakeBot` yang SENGAJA melempar `TelegramRetryAfter` pada panggilan tertentu untuk mereplikasi bug PERSIS): ronde tetap berhasil mulai walau kena flood control sekali di persis titik yang sama seperti laporan asli, jeda nyata terukur (bukan instan, benar-benar menunggu `retry_after`), sesi RUNNING berhasil dibatalkan lewat `cancel_game()` (pesan terkirim sebagai pesan baru, bukan edit), grup terbukti bisa `/game` lagi setelah dibatalkan, dan regresi: sesi `FINISHED` tetap ditolak `cancel_game()` seperti sebelumnya (bukan celah baru yang mengizinkan cancel status lain). Regresi penuh Tahap 2/3/4, susulan eliminasi, dan Force Start dijalankan ulang — semua tetap lolos.
