...
Bot Telegram jalan, bisa di-`python -m app.main` dari Windows dev. Fondasi (Fase 1-3 blueprint) **selesai** dan sudah dikeraskan (global error handler + recovery restart). Game pertama yang sesungguhnya (**Kursi Kosong**): Tahap 0-3 **sudah selesai** — game bisa dimainkan sungguhan dari `/game kursi_kosong` sampai ada pemenang, dengan jendela kontes kursi berbobot, status `AFK` vs `ELIMINATED` yang benar, reminder countdown 5/3 detik, dan narasi ronde final. Belum pernah dicoba jalan di Termux/Android TV Box sama sekali (Fase 5 blueprint nol persen).
...
|---|---|---|
| Kursi Kosong — Tahap 4 (skor & statistik) | ❌ | Tahap 0-3 selesai (lihat tabel "SUDAH jalan"). **Kandidat kerja selanjutnya yang paling jelas** — migration `user_game_scores`, `calculate_scores()` (hasil+partisipasi+ketahanan), AFK menghanguskan skor, commit idempoten. Ikuti `kursi-kosong-implementation-plan.md` Tahap 4 |
| Engine: sistem skor/leaderboard lintas-game | ❌ | Prasyarat Tahap 4 Kursi Kosong (skor hasil+partisipasi+ketahanan, digabung skor global). Bentuk tabel `user_game_scores` & hook `calculate_scores()` sudah disepakati (`game-development-guide.md` §15, `kursi-kosong-implementation-plan.md` Tahap 0.4) — belum ada migration/kode |
...
Tahap 0-3 sudah selesai (diverifikasi lewat integration test nyata — lihat `development-history.md`). Lanjut ke **Tahap 4** di [`kursi-kosong-implementation-plan.md`](kursi-kosong-implementation-plan.md): migration Alembic untuk `user_game_scores`, `KursiKosongGame.calculate_scores()` (skor hasil + partisipasi + ketahanan × faktor jumlah pemain), aturan AFK menghanguskan skor, commit idempoten di `GameManager.finish_game()`.
...
**Kalau lanjut Tahap 4:** baca `game-development-guide.md` §15 (skema skor & hook `calculate_scores()` sudah disepakati) — status `AFK` vs `ELIMINATED` yang jadi prasyaratnya sudah benar sejak Tahap 3, tinggal dipakai di formula skor (`player.status pernah AFK` → skor akhir 0, sesuai `game-design-kursi-kosong.md` §19).
