...
Bot Telegram jalan, bisa di-`python -m app.main` dari Windows dev. Fondasi (Fase 1-3 blueprint) **selesai** dan sudah dikeraskan (global error handler + recovery restart). Game pertama yang sesungguhnya (**Kursi Kosong**): Tahap 0, **Tahap 1 (lobby → ronde dasar)**, dan **Tahap 2 (rebutan kursi sesungguhnya) sudah selesai** — game bisa dimainkan sungguhan dari `/game kursi_kosong` sampai ada pemenang, kursi sekarang punya jendela kontes 1,2 detik + peluang berbobot (bukan lagi first-click-wins). Belum pernah dicoba jalan di Termux/Android TV Box sama sekali (Fase 5 blueprint nol persen).
...
|---|---|---|
| Kursi Kosong — Tahap 3 (AFK, eliminasi bernuansa, narasi lengkap) | ❌ | Tahap 0-2 selesai (lihat tabel "SUDAH jalan"). **Kandidat kerja selanjutnya yang paling jelas** — bedakan status `AFK` vs `ELIMINATED`, lengkapi bank narasi (§43 desain), countdown 5s/3s, narasi ronde final. Ikuti `kursi-kosong-implementation-plan.md` Tahap 3 |
...
Tahap 0, Tahap 1, dan Tahap 2 sudah selesai (diverifikasi lewat integration test nyata — lihat `development-history.md`). Lanjut ke **Tahap 3** di [`kursi-kosong-implementation-plan.md`](kursi-kosong-implementation-plan.md): bedakan status `AFK` (tidak beraksi sama sekali) vs `ELIMINATED` (beraksi tapi kalah/kehabisan kursi), lengkapi bank narasi acak (§43 desain), countdown 5 detik/3 detik dalam ronde, narasi & pesan khusus ronde final.
...
**Kalau lanjut Tahap 3:** baca `game-development-guide.md` §11 (status `AFK` sudah ada di enum, tinggal dipakai) — status `AFK` perlu dibedakan dari `ELIMINATED` di `_resolve_round`/`resolve_round` (lihat `state.py`), berdasarkan apakah pemain sempat melakukan aksi valid (termasuk klik ke kursi yang sudah terisi, §10 desain) selama ronde berjalan.
