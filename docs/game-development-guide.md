...
| Feature registry (on/off fitur per grup) | **Sudah dibangun** (2026-08-06, buat kebutuhan modul `autoreply`) — tabel `features`/`group_features` + `app/services/feature_service.py::is_enabled()`. **TAPI modul `games` belum memakainya** — games saat ini masih selalu aktif untuk semua grup, tidak ada toggle per grup dari database. Kalau mau menambahkan toggle per grup ke game tertentu, reuse `feature_service.is_enabled()` yang sudah ada, jangan bikin mekanisme baru. |
...
- **`app/modules/devtools/`** — command `/p0`.."/p7" untuk admin ber-impersonasi jadi virtual player, supaya bisa uji game multiplayer sendirian. Mekanismenya lewat middleware persona yang meresolusi identitas "yang benar-benar bertindak" — ini alasan §4 mewajibkan `context.acting_user_id` daripada `callback.from_user.id` manual. Alat development, aktif di semua environment (bukan cuma dev), tidak perlu diubah saat menambah game baru.
  - **Persona juga bisa diaktifkan di chat PRIVAT admin sendiri dengan bot** (bukan cuma grup) — dibutuhkan game apa pun yang punya interaksi lewat DM (mis. Kuis Kenal: deep link `/start`, jawaban privat). Persona disimpan per `(chat_id, real_telegram_id)`, jadi persona di grup dan persona di chat privat admin adalah slot yang TERPISAH.
  - **Batasan platform, bukan bug**: satu akun Telegram asli cuma punya SATU chat privat dengan bot. Untuk uji multi-pemain yang butuh aksi privat (mis. semua pemain menjawab lewat DM di ronde yang sama), admin harus ganti persona SATU PER SATU secara bergantian (`/p2` → buka deep link → ketik jawaban → konfirmasi → `/p3` → ulangi), bukan paralel. Timer fase (`ANSWER_TIMEOUT_SECONDS`/`JUDGING_TIMEOUT_SECONDS` di Kuis Kenal, misalnya) adalah timer PER-RONDE yang dibagi semua pemain, bukan TTL per-orang — solo test lewat gonta-ganti persona cuma realistis untuk jumlah pemain kecil dalam jendela waktu itu, jangan salah sangka jadi bug kalau timeout kepencet saat solo test dengan banyak persona.
  - **Urutan registrasi router penting**: `app/bootstrap.py::register_modules()` mendaftarkan router `devtools` SEBELUM `games` dengan sengaja, supaya `/p0`-`/p7` selalu dapat giliran cek lebih dulu daripada handler pesan privat generik games (yang menangkap semua pesan privat, termasuk yang berbentuk command, selama pengirim punya konteks input privat aktif). Kalau urutan ini kebalik, admin yang sedang berperan sebagai virtual player dengan konteks jawab/nilai aktif tidak akan pernah bisa ganti persona (perintahnya keburu "ditelan" game). Jangan ubah urutan ini tanpa membaca `tests/modules/devtools/test_persona_routing.py` dulu.
