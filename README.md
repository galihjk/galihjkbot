...
| [`leaderboard-subscription-gate-implementation-plan.md`](docs/leaderboard-subscription-gate-implementation-plan.md) | Rencana gating leaderboard global lewat subscribe channel + `MaintenanceGate` — kode+test otomatis selesai, belum dites manual di Telegram | Mau lanjut/uji fitur subscribe channel leaderboard |
...
- **Leaderboard bulanan bersifat DESTRUKTIF secara sengaja** (dikonfirmasi user, lihat `development-history.md`): tiap tanggal 1, `user_game_scores` bulan itu diumumkan lalu **dihapus fisik** (tidak ada riwayat all-time), DAN `User`/`Group` yang tidak aktif >6 bulan ikut dihapus (grup CASCADE menghapus seluruh riwayat game-nya). Jangan "perbaiki" ini jadi soft-delete tanpa tanya user dulu — ini keputusan produk, bukan bug. Penghapusan skor ini berlaku untuk SEMUA user terlepas status subscribe channel (lihat poin berikutnya) — subscribe cuma soal siapa yang MUNCUL di pengumuman channel, bukan soal siapa yang direset.
- **Leaderboard GLOBAL (channel + `/leaderboard`) cuma menghitung user yang subscribe channel leaderboard** (`users.is_leaderboard_channel_subscribed`, dicek live lewat `bot.get_chat_member` — lihat `docs/leaderboard-subscription-gate-implementation-plan.md`). **Leaderboard GRUP (`/leaderboardgrup` + posting per-grup) TIDAK terpengaruh sama sekali** — keputusan produk, bukan bug kalau ada user hilang dari global tapi tetap muncul lengkap di leaderboard grupnya.
- Selama job pemeliharaan leaderboard bulanan berjalan (re-verifikasi subscribe → posting → reset), pembuatan game baru (`/game`, klik menu game) & autoreply **dibekukan sementara** lewat `MaintenanceGate` (`app/core/maintenance.py`) — kalau bot kelihatan "menolak main" sebentar di awal bulan, itu normal, bukan bug. Game yang SUDAH berjalan tidak ikut dibekukan, boleh selesai seperti biasa.
