...
| **Kursi Kosong Tahap 1** (`app/modules/games/implementations/kursi_kosong/`) | ✅ | 3-8 pemain, ronde 15 detik, kursi first-click-wins (belum contest window), validasi nomor ronde di callback DARI AWAL (beda dari `simple_game`), keyboard 2 kursi/baris dengan nama pemain yang sudah duduk, ter-update live tiap kali ada yang duduk. Didaftarkan tanpa syarat environment (game sungguhan). Pesan ronde lama ditutup (snapshot kursi final + tombol dilepas) dengan jeda 2 detik sebelum narasi hasil ronde dikirim, biar terasa "waktu habis". Pacing pesan dalam-game: jeda 2 detik tiap kali bot kirim >1 pesan berturutan (welcome→ronde, narasi→ronde berikutnya/pemenang), kursi/keyboard baru muncul lewat edit setelah jeda acak 2-4 detik dari teks ronde, timer 15 detik mulai dihitung SETELAH kursi muncul — semua khusus pesan dalam-game, tidak berlaku untuk pesan sistem (lobi/ready-check) |
